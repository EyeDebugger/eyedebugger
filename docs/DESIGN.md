...
Status: v0.3 · 2026-09-25 · phase 1 (MVP) complete; phase 2: DAP facade (P2-M1), collaboration (P2-M2), VS Code extension (P2-M3, P2-M5), .NET side helper (P2-M6), .NET dumps, heap and threads (P2-M7)
...
- Errors: `{code, message, hint}`; codes are stable (`NO_SESSION`, `NOT_STOPPED`, `LEASE_HELD`, `UNSUPPORTED_BY_ADAPTER`, `SIDE_EFFECTS`, `ATTACH_FAILED`, `NO_TEST_HOST`, `ANCHOR_NOT_FOUND`, `ANCHOR_AMBIGUOUS`, `NOT_DOTNET`, `DIAGNOSTICS_DISABLED`, `DIAGNOSTICS_TIMEOUT`, `HELPER_NOT_FOUND`, `HELPER_MISMATCH`, `HELPER_FAILED`, `DUMP_UNSUPPORTED`, `DUMP_RUNTIME_MISSING`, `DUMP_FAILED`, …). Exit codes map to classes: 1 usage (incl. `SIDE_EFFECTS`, `ANCHOR_*`), 2 state (incl. a target or dump that can't be inspected: `NOT_DOTNET`, `DIAGNOSTICS_*`, `DUMP_UNSUPPORTED`), 3 setup (incl. `ATTACH_FAILED`, `NO_TEST_HOST`, `HELPER_NOT_FOUND`, `HELPER_MISMATCH`, `DUMP_RUNTIME_MISSING`), 4 adapter or side helper (incl. `UNSUPPORTED_BY_ADAPTER`, `HELPER_FAILED`, `DUMP_FAILED`).
...
internal/artifacts/  private artefact directory: dumps (0700/0600, EYEDBG_HOME), fresh names, pruning, --out placement
...
helpers/dotnet/      C# side helper (ADR 0016): src/EyeDbg.DotnetHelper (Rpc/, Methods/, Counters/, Diagnostics/, Dumps/, Analysis/),
