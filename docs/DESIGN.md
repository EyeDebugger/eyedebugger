...
- **Trust:** a user manifest names commands eyedbg runs, so it is trusted like the user's shell configuration: never read from a project; on Unix its directory, the parent and the file (checked through the opened handle) must be the user's and writable by neither group nor others (OpenSSH's `st_mode & 022` rule); nothing runs at load; argv never goes through a shell; downloads are https, size-capped and SHA-256-checked before extraction (§11). A `connect`-transport adapter's socket lives in a directory only the same user can enter and is gone, with the listener, on every return path — no TCP fallback: a same-user guard on the adapter's own side can't be relied on cross-platform (ADR 0012).
...
stdio by default; ADR 0012): `lldb-dap-c`, `lldb-dap-cpp` and `lldb-dap-rust`, one language each,
...
first language on the **connect transport** (ADR 0012): `dlv dap` never speaks stdio, only a Unix
...
**More languages** (ADR 0012, run independently of phase 2's own sequencing): C, C++, Rust
...
descoped in the same task, each needing strictly more than schema 1 offers today — see ADR 0012's
follow-ups F1–F5.
...
- Delve dialing out over `AF_UNIX` on Windows (ADR 0012): Go has supported it since Windows 10
  ...
  if it fails, the fix is not a TCP fallback (that needs its own authentication design, ADR 0012's
...
- lldb-dap discovery (ADR 0012 follow-up F6): it is rarely on PATH by default (macOS Command Line
