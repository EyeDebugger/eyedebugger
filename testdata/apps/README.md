...
Lines the tests look up end in a `// marker: NAME` comment, so editing an app
doesn't break line numbers in the tests. Tests copy an app to a temporary
directory before building it (`copyApp` in `drivers/dotnet`), so builds never
write here; `bin/` and `obj/` are ignored anyway.
